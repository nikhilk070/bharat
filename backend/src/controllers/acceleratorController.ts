...
import { generateStartupProfile } from '../utils/ai';
import OpenAI from 'openai';

// Phase 1: Access & Invites
export const inviteStartup = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await sendEmail(
      email, 
      'You are invited to Bharat Accelerator', 
      `<p>Welcome to Bharat Accelerator. Your temp password is: <b>${password}</b></p>`
    );
...
export const requestAccess = async (req: Request, res: Response) => {
  try {
    ...
    if (user.startup) {
      ...
      (async () => {
        try {
          const aiProfile = await generateStartupProfile(questionnaireData);
...
export const acceptRequest = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const startup = await prisma.startup.findUnique({ where: { id }, include: { founder: true } });
    
    if (!startup) return res.status(404).json({ message: 'Startup not found' });
    if (startup.status !== 'REQUESTED') return res.status(400).json({ message: 'Startup is not in REQUESTED state' });
    ...
    await sendEmail(
      ...
      'Your Application to Bharat Accelerator is Approved', 
      `<p>Congratulations! Your startup has been accepted. Your temporary login password is: <b>${password}</b></p><p>Please log in at /login.</p>`
    );
...
export const getRequests = async (req: AuthRequest, res: Response) => {
  try {
    const requests = await prisma.startup.findMany({
...
export const onboardingChat = async (req: Request, res: Response) => {
  try {
    ...
    const schema = settings?.questionnaireSchema as any;
...
export const submitQuestionnaire = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const startup = await prisma.startup.findUnique({ where: { founderId: userId } });
    if (!startup) return res.status(404).json({ message: 'Startup not found' });
...
export const allocateTime = async (req: AuthRequest, res: Response) => {
  try {
...
export const addMOM = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (event.status !== 'COMPLETED') {
       ...
       await prisma.event.update({ where: { id: eventId }, data: { mom } });
    }
...
export const uploadDocument = async (req: AuthRequest, res: Response) => {
  try {
    const { startupId, heading, signatureRequired } = req.body;
    const fileUrl = req.file ? (req.file as any).path : null;
    ...
    const startup = await prisma.startup.findUnique({ where: { id: startupId }, include: { founder: true }});
    if (startup) {
       await sendEmail(startup.founder.email, 'New Document Uploaded', '<p>Please review your new document.</p>');
       // Emit realtime notification
       io.to(startup.founderId).emit('notification', {
         type: 'DOCUMENT_UPLOADED',
         message: `A new document "${heading}" was uploaded to your vault.`,
         documentId: doc.id
       });
    }
...
export const actionDocument = async (req: AuthRequest, res: Response) => {
  try {
    const id = req.params.id as string;
...
export const onboardDecision = async (req: AuthRequest, res: Response) => {
  try {
    const startupId = req.params.startupId as string;
    const { decision } = req.body; // 'ONBOARDED' or 'REJECTED'

    await prisma.startup.update({ where: { id: startupId }, data: { status: decision } });
...
export const reportProblem = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
...
export const getProblems = async (req: AuthRequest, res: Response) => {
  try {
    const problems = await prisma.problemReport.findMany({ include: { startup: true } });
...
export const getMyStartup = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    ...
    if (!settings) {
      ...
      const defaultSettings = await prisma.systemSettings.create({
        data: {
          ...
          questionnaireSchema: defaultSchema as any
        }
      });
...
export const updateSubAdminScopes = async (req: AuthRequest, res: Response) => {
  try {
    const id = req.params.id as string;
