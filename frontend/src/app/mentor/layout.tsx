...
import { 
  Users, CalendarDays, MessageSquare, Settings, 
  LogOut, Menu, ChevronRight, Bell, Search, Star
} from "lucide-react";
...
export default function MentorLayout({ children }: { children: React.ReactNode }) {
  ...
  const navGroups = [
    {
      title: "Vivachana",
      items: [
        { href: "/mentor", label: "Dashboard", icon: <Star size={18} />, exact: true },
        { href: "/mentor/startups", label: "My Startups", icon: <Users size={18} /> },
        { href: "/mentor/calendar", label: "Sessions", icon: <CalendarDays size={18} /> },
        { href: "/mentor/messages", label: "Discussions", icon: <MessageSquare size={18} /> },
      ]
    },
    {
      title: "Settings",
      items: [
        { href: "/mentor/settings", label: "Profile", icon: <Settings size={18} /> },
      ]
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex overflow-hidden selection:bg-blue-500/30">
      ...
      >
        <div className="h-20 flex items-center px-6 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-[0_0_15px_rgba(59,130,246,0.4)]">
              <span className="font-heading font-extrabold text-white text-sm">V</span>
            </div>
            <span className="font-heading font-bold text-lg tracking-widest uppercase">Vivachana</span>
          </div>
        </div>
              ...
              <div className="space-y-1">
                {group.items.map((link) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`${isActive ? 'text-blue-500' : 'text-muted-foreground group-hover:text-foreground'} transition-colors`}>
                          {link.icon}
                        </div>
                        {link.label}
                      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-20 bg-background/80 backdrop-blur-xl border-b border-border flex items-center justify-between px-6 flex-shrink-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs font-heading font-semibold uppercase tracking-widest text-muted-foreground">
              <span>Vivachana Network</span>
              <span className="text-muted-foreground/50">/</span>
              <span className="text-blue-500">{pathname.split('/').pop() || 'Dashboard'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex relative group">
              ...
              <input type="text" placeholder="Search startups..." className="bg-muted border border-border rounded-full pl-10 pr-4 py-2 text-sm text-foreground focus:outline-none focus:border-blue-500 transition-all w-64" />
            </div>
            <ThemeToggle />
        ...
        <main className="flex-1 overflow-y-auto relative bg-background p-6 md:p-10">
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-transparent to-transparent pointer-events-none z-0"></div>
